...
class RemoveTeamMemberUseCase {
  ...
  async execute(teamId, targetUserId, performingUserId) {
    ...
    if (memberToRemove.role === TeamRole.OWNER) {
      ...
      throw new BadRequestError('Cannot remove the team owner. Change ownership first.');
    }
    ...
    if (performingUserId === targetUserId) {
      ...
      if (performingUserMembership.role === TeamRole.MEMBER) {
        ...
        throw new ForbiddenError('You are not authorized to remove this member. Requires Admin or Owner role.');
      }
    ...
    try {
      ...
      if (deletedCount === 0) {
        ...
        throw new InternalServerError('Failed to remove team member.');
      }
      ...
      return { message: 'Team member removed successfully.' };
        ...
        throw new InternalServerError(`Failed to remove member: ${error.message}`);
    }
  }
}
