...
class AdminUpdateUserUseCase {
  ...
  async execute(targetUserId, adminUpdateData, adminUserId) {
    ...
    if (!adminUserId) {
        ...
        throw new ApiError(httpStatusCodes.INTERNAL_SERVER_ERROR, 'Admin user ID not provided for action.');
    }

    // Fetch the user to be updated
    const userToUpdate = await this.userRepository.findById(targetUserId);
    if (!userToUpdate) {
      throw new ApiError(httpStatusCodes.NOT_FOUND, 'Target user not found.');
    }
    ...
    //   throw new ApiError(httpStatusCodes.FORBIDDEN, "Admins cannot change their own role or unverify themselves through this endpoint. Use specific admin management tools if needed.");
    // }
    ...
    if (!updatedUser) {
      throw new ApiError(httpStatusCodes.INTERNAL_SERVER_ERROR, 'Failed to update user by admin.');
    }
