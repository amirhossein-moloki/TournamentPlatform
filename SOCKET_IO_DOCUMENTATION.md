...
3.  **ذخیره اطلاعات کاربر**:
    *   در صورت موفقیت‌آمیز بودن احراز هویت، اطلاعات کاربر که از توکن استخراج شده (مانند `id` که همان `sub` در توکن است، `email` و `role`) در شیء `socket.user` ذخیره می‌شود.
    *   این اطلاعات در ادامه توسط handlerهای مختلف برای شناسایی کاربر و انجام عملیات مجاز مورد استفاده قرار می‌گیرد.
...
3.  **`sendMessage`**
    ...
    *   **عملکرد**:
        ...
        *   (مفهومی) در آینده می‌تواند پیام را در دیتابیس ذخیره کند.
        *   رویداد `newMessage` با شیء پیام ایجاد شده، به تمام کلاینت‌های حاضر در `roomId` (شامل خود فرستنده) ارسال می‌شود (`io.to(roomId).emit('newMessage', messageData)`).
        *   یک پیام لاگ در سرور ثبت می‌شود.
...
1.  **`newMessage`**
    *   **توضیحات**: این رویداد توسط سرور به تمام کلاینت‌های یک اتاق ارسال می‌شود وقتی پیام جدیدی در آن اتاق ارسال شده باشد.
    *   **Payload خروجی**:
        ```json
        {
          ...
          "sender": {
            "id": "string",     // شناسه کاربر فرستنده
            "username": "string"// نام کاربری فرستنده
          },
...
**نکات و توسعه‌های آینده (مفهومی):**

*   **ذخیره‌سازی پیام‌ها**: کد شامل کامنت‌هایی برای اتصال به `ChatMessageRepository` و ذخیره پیام‌ها در دیتابیس است که در حال حاضر پیاده‌سازی نشده.
*   **مجوز دسترسی به اتاق**: بررسی اینکه آیا کاربر مجاز به پیوستن به یک اتاق خاص است (مثلاً بر اساس عضویت در مسابقه).
*   **بارگذاری تاریخچه پیام**: ارسال پیام‌های اخیر اتاق به کاربری که تازه به اتاق پیوسته است.
...
1.  **`subscribeToTournamentUpdates`**
    ...
    *   **عملکرد**:
        ...
        *   یک پیام لاگ در سرور ثبت می‌شود.
        *   یک پاسخ (callback) به کلاینت ارسال‌کننده با وضعیت موفقیت یا شکست ارسال می‌شود.
        *   (مفهومی) پس از اشتراک موفق، سرور می‌تواند وضعیت فعلی مسابقه/براکت را برای کاربر ارسال کند.
    *   **Callback به کلاینت**:
        ```json
        // در صورت موفقیت
...
2.  **`bracketUpdate`**
    *   **توضیحات**: این رویداد برای اطلاع‌رسانی تغییرات در براکت مسابقه ارسال می‌شود، مانند شروع یک بازی، ثبت نتیجه یک بازی، و مشخص شدن بازی بعدی.
    *   **Payload خروجی (نمونه)**:
        ```json
        {
          "type": "MATCH_UPDATE", // یا نوع دیگری از آپدیت براکت
          "tournamentId": "string",
          "match": { // اطلاعات بازی آپدیت شده
            "id": "string",
            "status": "string", // (e.g., PENDING, ONGOING, COMPLETED)
            "participants": [ { "id": "string", "score": "number" }, ... ],
            "winnerId": "string" // (if completed)
          },
          "nextMatch": { ... } // (اختیاری) اطلاعات بازی بعدی که تحت تأثیر این آپدیت قرار گرفته
        }
        ```

3.  **`tournamentStatusUpdate`**
    *   **توضیحات**: این رویداد برای اطلاع‌رسانی تغییر در وضعیت کلی مسابقه ارسال می‌شود (مثلاً از "ثبت‌نام باز" به "در حال برگزاری").
    *   **Payload خروجی (نمونه)**:
        ```json
        {
          "tournamentId": "string",
...
*   **اتاق‌های اختصاصی**: هر مسابقه یک اتاق Socket.IO اختصاصی با فرمت `tournament:<tournamentId>` دارد. این امکان را فراهم می‌کند که آپدیت‌ها فقط برای کاربرانی ارسال شود که به آن مسابقه خاص علاقه‌مند هستند.
*   **ارسال آپدیت‌ها از لایه Application**: منطق اصلی که منجر به ارسال آپدیت‌های مسابقات می‌شود (مانند `broadcastBracketUpdate` یا `broadcastTournamentStatusChange` که به صورت مفهومی در کد handler اشاره شده‌اند) باید از لایه‌های بالاتر برنامه (use caseها یا سرویس‌ها) فراخوانی شود. برای مثال، وقتی یک use case نتیجه یک بازی را تایید می‌کند، سپس تابع مربوط به پخش آپدیت براکت را صدا می‌زند.
*   **مجوز دسترسی (Authorization)**: بررسی اینکه آیا کاربر مجاز به اشتراک در آپدیت‌های یک مسابقه خاص است (مثلاً آیا مسابقه عمومی است یا کاربر باید شرکت‌کننده باشد) یک نکته مهم است که باید در پیاده‌سازی واقعی لحاظ شود.
*   **آپدیت‌های پیچیده‌تر**: برای تعاملات آنی پیچیده‌تر، مانند گزارش زنده امتیازات توسط شرکت‌کنندگان در یک بازی، ممکن است نیاز به رویدادها و اتاق‌های بیشتری (مثلاً `match:<matchId>`) باشد.
